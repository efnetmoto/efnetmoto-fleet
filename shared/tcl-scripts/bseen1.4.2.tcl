...
set bs(path) "data/seen/"

###### Don't edit below here, even if you do know Tcl ######


#this is to fix a bind I didn't intend to use in a prev version (screwed up msg'd help). Sorry!
if {[bind msg -|- help] != "*msg:help"} {
...
proc bs_flood_init {} {
    ...
    if {![string match *:* $bs(flood)]} {
        putlog "$bs(version): var bs(flood) not set correctly."
        return
    }
...
proc bs_flood {nick uhost} {
    ...
    if {[unixtime] - $bs_flood_array([expr {$bs(flood_num) - 1}]) <= $bs(flood_time)} {
        putlog "$bs(version): Flood detected from $nick."
        if {$bs(ignore)} {
            newignore [join [maskhost *!*[string trimleft $uhost ~]]] \
                $bs(version) flood $bs(ignore_time)
        }
        return 1
    } {return 0}
}
if {[lsearch -exact [bind time -|- "*2 * * * *"] bs_timedsave] > -1} {
    unbind time -|- "*2 * * * *" bs_timedsave
} ;#backup frequency can be lower
proc bs_read {} {
    ...
    if {![file exists $bs(path)bs_data.$name]} {
        if {![file exists $bs(path)bs_data.$name.bak]} {
            putlog "     Old seen data not found!"
            putlog "     If this is the first time you've run the script, don't worry."
            putlog "     If there *should* be a data file from past runs of this script... worry."
            return
        } {
            exec cp $bs(path)bs_data.$name.bak $bs(path)bs_data.$name
            putlog "     Old seen data not found! Using backup data."
        }
    }
    ...
    while {![eof $fd]} {
        ...
        if {
            ![info exists bsu_version] || $bsu_version == "" ||
            $bsu_version < $bs(updater)
        } {
            ...
            if {$temp || [bsu_go] || [bsu_finish]} {
                ...
                putlog "It is *not* safe to run the bot w/ a bseen database \
                    that is not matched to this version of bseen."
                putlog "If you can't find the problem, the only option is to remove \
                    the bs_data.$name and bs_data.$name.bak files.  Then restart the bot."
                putlog "Because this is a potential crash point in the bot, the bot will now halt."
...
if {![info exists bs_list] || [array size bs_list] == 0} {
    putlog "     Loading seen database..."
    bs_read
}
###
...
proc bs_save {} {
    ...
    if {[file exists $bs(path)bs_data.$name]} {
        catch {exec cp -f $bs(path)bs_data.$name $bs(path)bs_data.$name.bak}
    }
    set fd [open $bs(path)bs_data.$name w]
    set id [array startsearch bs_list]
    putlog "Backing up seen data..."
    puts $fd "#$bs(updater)"
...
proc bs_kick {nick uhost hand chan knick reason} {
    bs_add $knick "[getchanhost $knick $chan] [unixtime] kick $chan [list $nick] [list $reason]"
}
bind nick -|- * bs_nick
...
bind chon -|- * bs_chon
proc bs_chon {hand idx} {
    foreach item [dcclist] {
        if {[lindex $item 3] != "CHAT"} {continue}
        if {[lindex $item 0] == $idx} {bs_add $hand "[lindex $item 2] [unixtime] chon"; break}
    }
}
if {[lsearch -exact [bind chof -|- *] bs_chof] > -1} {
    unbind chof -|- * bs_chof
} ;#this bind isn't needed any more
bind chjn -|- * bs_chjn
...
proc bs_chpt {bot hand args} {
    ...
    if {$old != "0"} {bs_add $hand "[join [string trim [lindex $old 1] ()]] [unixtime] chpt $bot"}
}
...
proc bs_away {bot idx msg} {
    ...
    set old [split [bs_search ? [string tolower $hand]]]
    if {$old != "0"} {
        set fmsg [bs_filt [join $msg]]
        bs_add $hand "[join [string trim [lindex $old 1] ()]] [unixtime] away $bot $fmsg"
    }
}
...
proc bs_botsearch {from cmd args} {
    ...
    set command [lindex $args 0]; set target [lindex $args 1]; set nick [lindex $args 2]
    set search [bs_filt [join [lrange $args 3 e]]]
    if {[string match *\\\** $search]} {
        set output [bs_seenmask bot $nick $search]
        if {$output != "No matches were found." && ![string match "I'm not on *" $output]} {
            set fout [bs_filt $output]
            putbot $from "bs_botsearch_reply $command \{$target\} {$nick, $botnick says:  $fout}"
        }
    } {
        set output [bs_output bot $nick [bs_filt [lindex $search 0]] 0]
        if {$output != 0 && [lrange [split $output] 1 4] != "I don't remember seeing"} {
            set fout [bs_filt $output]
            putbot $from "bs_botsearch_reply $command \{$target\} {$nick, $botnick says:  $fout}"
        }
    }
...
proc bs_botsearch_reply {from cmd args} {
    ...
    if {![info exists bs(bot_delay)]} {
        ...
        if {![lindex $args 0]} {
            putdcc [lindex $args 1] "[join [lindex $args 2]]"
...
proc bs_dccreq {hand idx args no} {
    ...
    if {[lrange [split $output] 1 4] == "I don't remember seeing"} {
        putallbots "bs_botsearch 0 $idx $hand $args"
    }
    putdcc $idx "$output"; return $bs(logqueries)
}
...
proc bs_msgreq {nick uhost hand args no} {
    ...
    if {[string match *\\\** [lindex $args 0]]} {
        ...
        if {$output == "No matches were found."} {
            putallbots "bs_botsearch 1 \{notice $nick\} $nick $args"
        }
        if {[string match "I'm not on *" $output]} {
            putallbots "bs_botsearch 1 \{notice $nick\} $nick $args"
        }
...
proc bs_pubreq {nick uhost hand chan args no} {
    ...
    if {
        $bs(log_only) != "" &&
        [lsearch -exact $bs(log_only) [string tolower $chan]] == -1
    } {return 0}
    set args [bs_filt [join $args]]
    if {[lsearch -exact $bs(quiet_chan) [string tolower $chan]] >= 0} {
        set target "notice $nick"
    } {set target "privmsg $chan"}
    if {[string match *\\\** [lindex $args 0]]} {
        set output [bs_seenmask $chan $hand $args]
        if {$output == "No matches were found."} {
            putallbots "bs_botsearch 1 \{$target\} $nick $args"
        }
        if {[string match "I'm not on *" $output]} {
            putallbots "bs_botsearch 1 \{$target\} $nick $args"
        }
    ...
    if {[string tolower $nick] == [string tolower $data]} {
        puthelp "$target :$nick, go look in a mirror."
        return $bs(logqueries)
    }
    if {[string tolower $data] == [string tolower $botnick]} {
        puthelp "$target :$nick, I'm right here.  Quit wasting my time!"
        return $bs(logqueries)
    }
    if {[onchan $data $chan]} {
        puthelp "$target :$nick, $data is right here!"; return $bs(logqueries)
    }
...
proc bs_output {chan nick data no} {
    ...
    if {$data == [string tolower $botnick]} {
        return [concat $nick, I'm right here. Quit wasting my time!]
    }
    ...
    if {$no == 0} {
        ...
        if {[lsearch -exact [array names bs_list] $data] != "-1"} {
            ...
            foreach item [bs_seenmask dcc ? [maskhost $addy]] {
                if {[lsearch -exact $matches $item] == -1} {set matches "$matches $item"}
            }
        }
        if {[validuser $data]} {set hand $data}
        if {$hand != "*" && $hand != ""} {
            if {[string trimleft [lindex $version 1] 0] > 1030000} {
                set hosts [getuser $hand hosts]
            } {set hosts [gethosts $hand]}
            foreach addr $hosts {
                foreach item [string tolower [bs_seenmask dcc ? $addr]] {
                    if {[lsearch -exact [string tolower $matches] [string tolower $item]] == -1} {
                        set matches [concat $matches $item]
                    }
                }
            }
        }
        if {$matches != ""} {
            ...
            if {$len > 99} {
                return [concat I found $len matches to your query\; \
                    please refine it to see any output.]
            }
            ...
            if {$len <= 5} {
                ...
                set output [concat I found $len matches to your query. \
                    Here are the 5 most recent (sorted): [join [lrange $matches 0 4]].]
                set output [concat $output [bs_search $chan $key]]; return $output
            }
        }
    }
    ...
    if {$temp != 0} {return $temp} {
        ...
        if {![validuser [bs_filt $data]] || [string trimleft [lindex $version 1] 0] < 1030000} {
            ...
            if {
                ($chan != [lindex $seen 1] || $chan == "bot" || $chan == "msg" ||
                    $chan == "dcc") && [validchan [lindex $seen 1]] &&
                [lindex [channel info [lindex $seen 1]] 23] == "+secret"
            } {
                set chan "-secret-"
            } {
...
proc bs_search {chan n} {
    ...
    if {[lsearch -exact [array names bs_list] [string tolower $n]] != "-1"} {
        ...
        set n [join [lindex $data 0]]; set addy [lindex $data 1]
        set time [lindex $data 2]; set marker 0
        if {
            ([string tolower $chan] != [string tolower [lindex $data 4]] ||
                $chan == "dcc" || $chan == "msg" || $chan == "bot") &&
            [validchan [lindex $data 4]] &&
            [lindex [channel info [lindex $data 4]] 23] == "+secret"
        } {
            set chan "-secret-"
        } {
        ...
        switch -- [lindex $data 3] {
            part {
                ...
                set output [concat $n ($addy) was last seen parting $chan \
                    [bs_when $time] ago$reason]
            }
            quit {
                set output [concat $n ($addy) was last seen quitting from $chan \
                    [bs_when $time] ago stating ([join [lrange $data 5 e]]).]
            }
            kick {
                set output [concat $n ($addy) was last seen being kicked from $chan by \
                    [lindex $data 5] [bs_when $time] ago with the reason \
                    ([join [lrange $data 6 e]]).]
            }
            rnck {
                set output [concat $n ($addy) was last seen changing nicks from \
                    [lindex $data 5] on [lindex $data 4] [bs_when $time] ago.]
            ...
            nick {
                set output [concat $n ($addy) was last seen changing nicks to \
                    [lindex $data 5] on [lindex $data 4] [bs_when $time] ago.]
            }
            splt {
                set output [concat $n ($addy) was last seen parting $chan due to a split \
                    [bs_when $time] ago.]
            }
            rejn {
                set output [concat $n ($addy) was last seen rejoining $chan from a split \
                    [bs_when $time] ago.]
                if {[validchan $chan]} {
                    if {[onchan $n $chan]} {
            ...
            away {
                set reason [lrange $data 5 e]
                if {$reason == ""} {
                    set output [concat $n ($addy) was last seen returning to the partyline \
                        on $chan [bs_when $time] ago.]
                } {
                    set output [concat $n ($addy) was last seen being marked as away \
                        ($reason) on $chan [bs_when $time] ago.]
                }
            }
            chon {
                set output [concat $n ($addy) was last seen joining the partyline \
                    [bs_when $time] ago.]
            ...
            chof {
                set output [concat $n ($addy) was last seen leaving the partyline \
                    [bs_when $time] ago.]
                set lnick [string tolower $n]
                foreach item [whom *] {
                    if {$lnick == [string tolower [lindex $item 0]]} {
                        set output [concat $output $n is on the partyline in \
                            [lindex $item 1] still.]
                        break
                    }
                }
            ...
            chjn {
                set output [concat $n ($addy) was last seen joining the partyline on $chan \
                    [bs_when $time] ago.]
                set lnick [string tolower $n]
                foreach item [whom *] {
                    if {$lnick == [string tolower [lindex $item 0]]} {
            ...
            chpt {
                set output [concat $n ($addy) was last seen leaving the partyline from $chan \
                    [bs_when $time] ago.]
                set lnick [string tolower $n]
                foreach item [whom *] {
                    if {$lnick == [string tolower [lindex $item 0]]} {
...
proc bs_when {lasttime} {
    ...
    if {$time >= 31536000} {
        set years [expr {int($time / 31536000)}]
        set time [expr {$time - 31536000 * $years}]
    }
    if {$time >= 86400} {
        set days [expr {int($time / 86400)}]
        set time [expr {$time - 86400 * $days}]
    }
    if {$time >= 3600} {
        set hours [expr {int($time / 3600)}]
        set time [expr {$time - 3600 * $hours}]
    }
    if {$time >= 60} {set mins [expr {int($time / 60)}]}
    if {$years == 0} {set output ""} \
        elseif {$years == 1} {set output "1 year,"} {set output "$years years,"}
...
proc bs_add {nick data} {
    global bs_list bs
    if {
        [lsearch -exact $bs(no_log) [string tolower [lindex $data 3]]] >= 0 ||
        ($bs(log_only) != "" &&
            [lsearch -exact $bs(log_only) [string tolower [lindex $data 3]]] == -1)
    } {return}
...
proc bs_seenmask {ch nick args} {
    global bs_list bs
    set matches ""; set temp ""; set i 0; set args [join $args]; set chan [lindex $args 1]
    ...
    while {[array anymore bs_list $id]} {
        ...
        set i 0; set temp ""
        set match [lindex $bs_list($item) 0]; set addy [lindex $bs_list($item) 1]
        if {[string match $data $item![string tolower $addy]]} {
            set match [bs_filt $match]; if {$chan != ""} {
                if {[string match $chan [string tolower [lindex $bs_list($item) 4]]]} {
    ...
    if {$len > 99} {
        return "I found $len matches to your query; please refine it to see any output."
    }
    set matches [bs_sort $matches]
    if {$len <= 5} {
        set output [concat I found $len matches to your query (sorted): [join $matches].]
    } {
        set output "I found $len matches to your query.  Here are the 5 most recent \
            (sorted): [join [lrange $matches 0 4]]."
    }
    return [concat $output [bs_output $ch $nick [lindex [split $matches] 0] 1]]
}
proc bs_sort {data} {
    global bs_list
    set data [bs_filt [join [lsort -decreasing -command bs_lsortcmd $data]]]
    return $data
}
...
proc bs_pubstats {nick uhost hand chan args} {
    ...
    if {
        [bs_flood $nick $uhost] ||
        [lsearch -exact $bs(no_pub) [string tolower $chan]] >= 0 ||
        ($bs(log_only) != "" &&
            [lsearch -exact $bs(log_only) [string tolower $chan]] == -1)
    } {return 0}
    if {[lsearch -exact $bs(quiet_chan) [string tolower $chan]] >= 0} {
    ...
    puthelp "$target :[bs_stats]"; return 1
}
bind msg -|- seenstats bs_msgstats
proc bs_msgstats {nick uhost hand args} {
    global bs; if {[bs_flood $nick $uhost]} {return 0}
    puthelp "notice $nick :[bs_stats]"; return $bs(logqueries)
}
proc bs_stats {} {
    global bs_list bs
    set id [array startsearch bs_list]; set bs_record [unixtime]; set totalm 0; set temp ""
    while {[array anymore bs_list $id]} {
        set item [array nextelement bs_list $id]
        set tok [lindex $bs_list($item) 2]
        if {$tok == ""} {putlog "Damaged seen record: $item"; continue}
        if {[lindex $bs_list($item) 2] < $bs_record} {
            set bs_record [lindex $bs_list($item) 2]; set name $item
        }
        set addy [string tolower [maskhost [lindex $bs_list($item) 1]]]
        if {[lsearch -exact $temp $addy] == -1} {incr totalm; lappend temp $addy}
    }
    array donesearch bs_list $id
    return "Currently I am tracking [array size bs_list]/$bs(limit) nicks, \
        which comprise $totalm unique uhosts.  The oldest record is \
        [lindex $bs_list($name) 0]'s, which is from [bs_when $bs_record] ago."
}
bind dcc -|- chanstats bs_dccchanstats
proc bs_dccchanstats {hand idx args} {
    if {$args == "{}"} {set args [console $idx]}
    if {[lindex $args 0] == "*"} {
        putdcc $idx "$hand, chanstats requires a channel arg, or a valid console channel."
        return 1
    }
...
proc bs_pubchanstats {nick uhost hand chan args} {
    ...
    if {
        [bs_flood $nick $uhost] || [lsearch -exact $bs(no_pub) $chan] >= 0 ||
        ($bs(log_only) != "" &&
            [lsearch -exact $bs(log_only) [string tolower $chan]] == -1)
    } {return 0}
    if {[lsearch -exact $bs(quiet_chan) $chan] >= 0} {
        set target "notice $nick"
    } {set target "privmsg $chan"}
    if {[lindex $args 0] != ""} {set chan [lindex $args 0]}
    puthelp "$target :[bs_chanstats $chan]"; return $bs(logqueries)
}
bind msg -|- chanstats bs_msgchanstats
proc bs_msgchanstats {nick uhost hand args} {
    global bs; if {[bs_flood $nick $uhost]} {return 0}
    puthelp "notice $nick :[bs_chanstats [lindex $args 0]]"; return $bs(logqueries)
}
proc bs_chanstats {chan} {
    global bs_list
    set chan [string tolower $chan]; if {![validchan $chan]} {return "I'm not on $chan."}
    set id [array startsearch bs_list]; set bs_record [unixtime]
    set totalc 0; set totalm 0; set temp ""
    while {[array anymore bs_list $id]} {
        set item [array nextelement bs_list $id]
        set time [lindex $bs_list($item) 2]; if {$time == ""} {continue}
    ...
    return "$chan is the source of [expr {100 * $totalc / $total}]% ($totalc/$total) of the \
        seen database entries.  In $chan, there were a total of $totalm unique uhosts \
        seen in the last [bs_when $bs_record]."
}
foreach chan [string tolower [channels]] {
    if {![info exists bs_botidle($chan)]} {set bs_botidle($chan) [unixtime]}
}
...
proc lastspoke {nick uhost hand chan args} {
    ...
    if {
        [bs_flood $nick $uhost] || [lsearch -exact $bs(no_pub) $chan] >= 0 ||
        ($bs(log_only) != "" && [lsearch -exact $bs(log_only) $chan] == -1)
    } {return 0}
    if {[lsearch -exact $bs(quiet_chan) $chan] >= 0} {
        set target "notice $nick"
    } {set target "privmsg $chan"}
    ...
    if {[string match *\** $data]} {
        ...
        if {[lsearch -glob $chanlist $ldata] > -1} {
            set data [lindex [chanlist $chan] [lsearch -glob $chanlist $ldata]]
        }
    }
    if {[onchan $data $chan]} {
        if {$ldata == [string tolower $botnick]} {
            puthelp "$target :$nick, must you waste my time?"; return 1
        }
        set time [getchanidle $data $chan]
        set bottime [expr {([unixtime] - $bs_botidle($chan)) / 60}]
        if {$time < $bottime} {
            if {$time > 0} {
                set diftime [bs_when [expr {[unixtime] - $time * 60 - 15}]]
            } {set diftime "less than a minute"}
...
proc bs_help_dcc {hand idx args} {
    ...
    switch -- $args {
        seen {
            putidx $idx "###  seen <query> \[chan\]          $bs(version)"
            putidx $idx "   Queries can be in the following formats:"
            putidx $idx "     'regular':  seen lamer; seen lamest "
            putidx $idx "     'masked':   seen *l?mer*; seen *.lame.com; seen *.edu #mychan"
        }
        seennick {putidx $idx "###  seen <nick>          $bs(version)"}
        chanstats {
            putidx $idx "###  chanstats <chan>"
            putidx $idx "   Returns the usage statistics of #chan in the seen database."
        }
        seenstats {
            putidx $idx "###  seenstats          $bs(version)"
            putidx $idx "   Returns the status of the bseen database."
        }
        unseen {
            if {[matchattr $hand n]} {
                putidx $idx "###  unseen <chan>          $bs(version)"
                putidx $idx "   Deletes all <chan> entries from the bseen database."
            }
        }
